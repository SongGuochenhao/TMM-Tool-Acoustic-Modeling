% demo_ElasticFoam_AlPanel   Predict the absorption and transmission
...
foam_nu = 0.33;
% stiff panel - Al
etast = 0.003;                 % loss factor
E0st = 70*10^9;                % [Pa] Static Young's modulus
Est = E0st*(1+1i*etast);        % complex Young's modulus
vst = 0.33;                    % Poission's ratio
Rost = 2700;                   % [kg/m^3] density
hp = 1e-3;                    % [m] thickness of the panel
D = Est*hp^3/12/(1-vst^2);     % [Pa*m^3] flexural stiffness per unit width - front panel
Dp = Est*hp;                   % [Pa*m] longitudinal stiffness per unit width - front panel
ms = Rost*hp;                  % [kg/m^2] mass per unit area - front panel


% BC matrices
[B1_pos, B1_neg] = fun_bc('fluid','poro',foam_phi);
[B2_pos, B2_neg] = fun_bc('poro','stiff panel',foam_phi);
[B3_pos, B3_neg] = fun_bc('stiff panel','fluid');
...
for count = 1:length(w)
    ...
    % replace with a equivalent interface
    [B_pos,B_neg] = fun_merge_layer(B1_pos,B1_neg,B2_pos, B2_neg, Phi, Lambda);
    % lower layer - Al stiff panel
    [Phi,Lambda] = fun_TM_panel(w(count), c0, hp, ms, Dp, D, theta);
    d = hp+foam_h;
    % 2x2 TM for the whole system
    TM(:,:,count) = fun_1layer_pred(B_pos,B_neg,B3_pos, B3_neg, Phi, Lambda);
